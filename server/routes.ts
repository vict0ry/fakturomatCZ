...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/invoices/:id/items", requireAuth, async (req: any, res) => {
    try {
      ...
      // Log item creation to history
      const invoice = await storage.getInvoice(invoiceId, req.user.companyId);
      if (invoice) {
        await storage.createInvoiceHistory({
          invoiceId: invoiceId,
          companyId: req.user.companyId,
          userId: req.user.id,
          action: 'item_added',
          description: `Položka "${itemData.description}" byla přidána (${itemData.quantity} ${itemData.unit} za ${itemData.unitPrice})`,
  ...
  app.patch("/api/invoice-items/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      // Get old item data before update
      const oldItem = await storage.getInvoiceItem(id);
      if (!oldItem) {
      ...
      // Log item update to history
      const invoice = await storage.getInvoice(oldItem.invoiceId, req.user.companyId);
      if (invoice) {
        const changedFields = Object.keys(itemData).filter(key => itemData[key] !== undefined);
        await storage.createInvoiceHistory({
          invoiceId: oldItem.invoiceId,
          companyId: req.user.companyId,
          userId: req.user.id,
          action: 'item_updated',
          description: `Položka "${oldItem.description}" byla upravena (${changedFields.join(', ')})`,
          metadata: JSON.stringify({
            itemId: id,
            changedFields,
            oldValues: changedFields.reduce((acc, field) => {
              acc[field] = (oldItem as any)[field];
              return acc;
            }, {} as any),
            newValues: itemData,
            source: 'manual_edit'
  ...
  app.delete("/api/invoice-items/:id", requireAuth, async (req: any, res) => {
    try {
      ...
      const invoice = await storage.getInvoice(item.invoiceId, req.user.companyId);
      if (invoice) {
        await storage.createInvoiceHistory({
          ...
          userId: req.user.id,
          action: 'item_deleted',
          description: `Položka "${item.description}" byla smazána (${item.quantity} ${item.unit} za ${item.unitPrice})`,
          metadata: JSON.stringify({
            deletedItem: {
              description: item.description,
